...
class Statusbar:
    ...
    def update_status(self, event):
        if event == 'saved':
            ...
            self.display_status_message(
                'No file detected. Create or open a file.')
        elif event == 'no python':
            self.display_status_message('You can only run Python files.')
        elif event == 'no txt bold':
            self.display_status_message(
                'You can only bold text in text files.')
        elif event == 'no txt high':
            self.display_status_message(
                'You can only highlight text in text files.')
        elif event == 'zen':
            self.display_status_message(
                'You can leave zen mode by pressing "escape"', msg_type='hint')
        else:
    ...
    # hiding the status bar while in zen mode
