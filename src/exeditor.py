...
from ex_syntax_highlighting import PythonSyntaxHighlight
...
from ex_context import ContextMenu
from ex_zutilityfuncs import load_settings_data, store_settings_data


class ExEText(tk.Frame):
    def __init__(self, *args, **kwargs):
        ...
        self.syntax_highlighter = PythonSyntaxHighlight(
            self, self.textarea, self.initial_content)
    ...
    # editor zen mode calling which removes status bar and menu bar
    def enter_zen_mode(self, *args):
        ...
        self.statusbar.update_status('zen')
    ...
    def on_closing(self):
        message = tk.messagebox.askyesnocancel(
            "Save On Close", "Do you want to save the changes before closing?")
        if message == True:
            self.quit_save()
        elif message == False:
            quit()
    ...
    def run(self, *args):
        try:
            if self.filename[-3:] == '.py':
                ...
                if os.name == 'nt':
                    os.system(
                        f'start cmd.exe @cmd /k "python {self.filename}"')
                else:
    ...
    def change_font_size(self, delta):
        ...
        self.syntax_highlighter.text.tag_configure(
            "Token.Name.Builtin.Pseudo", font=self.italics)
    ...
    def bind_shortcuts(self, *args):
        ...
        text.bind('<Control-q>', self.enter_zen_mode)
        text.bind('<Control-f>', self.show_find_window)
        text.bind('<Escape>', self.leave_zen_mode)
        ...
        text.bind('<Shift-asciitilde>',
                  self.syntax_highlighter.initial_highlight)
        text.bind('<Control-Shift-KeyRelease>',
                  self.syntax_highlighter.initial_highlight)
